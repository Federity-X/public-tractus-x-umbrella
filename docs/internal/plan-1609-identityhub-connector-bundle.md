...
> **Status of this plan:** **2026-06-20 — ALL #1609 VALIDATION STEPS COMPLETE; 2026-06-21 — hardening pass done.** Full DCP data-exchange (catalog → negotiation → transfer → fetch) validated end-to-end on **all three** IdentityHub profiles (shared, per-participant, persistent/postgres). See **section 0.2** for the authoritative current standing, **section 0.2.1** for the hardening pass, and **section 0.4** for the paste-ready PR-positioning note. (section 0.2 supersedes the "What's left" / G2 items in section 0.1; section 0.1 in turn supersedes the gaps/DoD/version facts further below.)
> **Re-plan history:** 2026-06-19 — Test Case 1 passes on the shared profile (**section 0.1**). 2026-06-18 — latest-component check + per-participant feasibility (**section 0.0**). 2026-04-19 draft v3 — D10 resolved (postgres IssuerService); see `docs/common/concept/1609-local-test-findings.md#phase-11`.

---

## 0.2 — 2026-06-20 update: all validation steps complete (authoritative)

**Supersedes the "What's left" list and the G2 gap row in section 0.1.** Since section 0.1 the
...
| ------------------------------------------------------------- | --------------------------------- | ------------------------------------------- |
| `values-test-data-exchange-identity-hub.yaml`                 | shared in-memory IH               | DONE clean `helm install` from scratch        |
| `values-test-data-exchange-identity-hub-per-participant.yaml` | one IH per participant (2: provider + consumer1) | DONE provider↔consumer1, each on its own IH   |
| `values-test-data-exchange-identity-hub-postgres.yaml` (NEW)  | persistent IH (PostgreSQL)        | DONE full transfer                            |

All three run on the EDC-0.17.0-aligned stack via the new image overlay
`values-test-data-exchange-identity-hub-local-0.17.0.yaml` (see section 0.1 for the
version gap — the committed `Chart.yaml`s still pin the 0.16.0 line).

**Done since section 0.1:**
...
2. **Per-participant topology (G2) — DONE** (was "follow-up" in section 0.1). Each
...
5. **Committed** to `feature/1609-identityhub-connector-bundle` as a series of
   ...
   sign-off and no AI attribution; local-only config files excluded. (The hardening pass in
   section 0.2.1 and section 0.3/section 0.4 landed in later commits on the same branch.)

Also hardened since section 0.1: the BDRS directory-seeding hook now runs on
...
**section 0.4** for the paste-ready PR-positioning note.
...
Complementary, not overlapping — see section 0.3. #412 is a release-pinned
...
| ----------------------------------- | ------------------ | -------------------------------------- | ----------------------------------------------------------------------------- |
| `tractusx-identityhub-memory`       | **v0.3.2**         | **v0.3.2** (2026-06-16)                | DONE already newest                                                             |
| `tractusx-issuerservice` (postgres) | **v0.3.2**         | **v0.3.2**                             | DONE already newest                                                             |
| `tractusx-connector`                | **0.13.0-rc2**     | `0.12.1` stable · `0.13.0` still `-rc` | NOTE on an RC ahead of stable (kept on purpose — DCP-native, matches IH 0.16.0) |
| `bdrs-server-memory`                | 0.6.0              | 0.6.0                                  | DONE newest                                                                     |
| `digital-twin-registry`             | 0.10.0             | 0.12.0                                 | BEHIND 2 minors behind (optional housekeeping)                                    |
| `ssi-dim-wallet-stub`               | 0.1.14             | 0.1.18                                 | BEHIND behind (default path; optional)                                            |
...
**Concrete per-participant work (supersedes the old "Phase D / G2 = follow-up" framing in section 4/section 5):**
...
**This section supersedes the gap analysis (section 4 G1–G6), the runtime assumptions
(section 11.4 R1–R3), the Definition of Done (section 8), and the connector/IH version facts
in section 0/section 0.0/section 2.** Everything below was proven on a live `kind` cluster
...
| Item | Was | Now |
| ---- | --- | --- |
| G1 single source of truth for BPN/DID/keys | HIGH | DONE `_wallet-derive.tpl` + `wallet.participants`; seed drives everything from it |
| G2 per-participant topology | MEDIUM | TODO still shared-only validated; per-participant profile exists, not E2E-tested (follow-up) |
| G3 seeding hook E2E-verified | HIGH | DONE all participants seeded, all credentials `ISSUED`, full transfer green |
| G4 connector↔IH STS mismatch | HIGH | DONE STS works (SI-token mint + VP pull both directions, rich claims) |
| G5 dedicated feature branch | LOW | DONE on `feature/1609-identityhub-connector-bundle` |
| G6 user deploy guide | MEDIUM | PARTIAL `docs/user/common/guides/data-exchange-identity-hub.md` exists; needs a refresh for the 0.17.0 stack + smoke test |
| section 11.4 R1 DID doc CredentialService entry | runtime-check | DONE present (plain ctxId) |
| section 11.4 R2 STS wire compatibility | tail risk | DONE compatible on EDC 0.17.0 |
| section 11.4 R3 issuance completes | runtime-check | DONE all VCs reach `ISSUED` |
...
3. **Per-participant topology (G2).** Wire + E2E-test the per-participant profile
   (4 IH pods) — section 0.0 has the steps. Follow-up, not a 26.06 blocker.
...
5. **Commit hygiene.** Working tree has the seed enrichment + repackaged
   ...
   (single-author; keep local-only config + work-planning files out of commits). Revert any live
...
> **NOTE Everything from section 0 onwards is the ORIGINAL plan (2026-04 → 2026-06-18), kept for history.** Where it conflicts with section 0.1 / section 0.2 above — e.g. `tractusx-connector 0.11.2` + the `iatp` / `sts.dim` schema (now `0.13.0-rc2`, DCP-native, `dcp` keys), "shared-only / per-participant deferred to 26.09" (per-participant + postgres are done), or "Test Case 1 unproven" (it passes) — **section 0.1 / section 0.2 are authoritative.**
...
- `tractusx-identityhub-memory` **v0.3.2** (2026-06-16) — _was v0.2.0 when this plan was drafted; see section 0.0_
...
cluster before opening the PR** (section 11 _Validation_). If any of them fail, the
...
## 2. Upstream state (verified 2026-04-18; **refreshed 2026-06-18 — see section 0.0**)
...
> (EDC 0.16.0). See section 0.0 for the up-to-date version matrix.
...
Branch: `feature/1609-identityhub-connector-bundle` (re-scoped from the original `feature/BE-165-umbrella-chart-documentation` — see section 7).
...
| **G2. Per-participant topology only documented, not wired.** `tx-data-provider`/`dataconsumerOne`/`dataconsumerTwo` have no IH sub-dep; the per-participant profile currently can't deploy. **This is an umbrella Helm-composition task, not a component-version one** — see section 0.0 for the concrete wiring steps (we are already on the newest IH/IS). | The word "decentralized" in benefit #1 implies each participant runs its own IH. | **MEDIUM** — shared mode covers the test-case. |
...
1. Install minikube (see section 6.1), run the shared-topology profile.
...
- Consult Phase B section 4 (STS mismatch fallback).
...
| R2  | `tractusx-connector 0.11.2` can't obtain SI tokens from IH's STS because the connector expects OAuth2. | Phase B section 4 — embedded STS fallback documented in the profile.                                           |
...
| R5  | IngressDNS entries drift across macOS / Linux / Windows.                                               | section 6.1 keeps `/etc/hosts` as the source of truth; ingress-dns is optional.                                |
...
| -------------------------------------------------------------------------------- | -------------------------------------- | ------------------------------- |
| `POST /api/identity/v1alpha/participants` (create ParticipantContext)            | IH v0.2.0 + IS v0.2.0                  | DONE Present                      |
| `PUT /api/identity/v1alpha/participants/{id}/state?isActive=true`                | IH + IS v0.2.0                         | DONE Present                      |
| `GET /.well-known/did.json` served by IH's DID endpoint (port 8083)              | IH v0.2.0                              | DONE Present                      |
| `POST /api/admin/v1alpha/participants/{id}/attestations`                         | IS v0.2.0                              | DONE Present                      |
| `POST /api/admin/v1alpha/participants/{id}/credentialdefinitions`                | IS v0.2.0                              | DONE Present                      |
| `POST /api/admin/v1alpha/participants/{id}/holders`                              | IS v0.2.0                              | DONE Present                      |
| `POST /api/identity/v1alpha/participants/{id}/credentials/request` (DCP trigger) | IH v0.2.0                              | DONE Present                      |
| `POST /api/sts` (OAuth2 token + SI-token mint)                                   | IH v0.2.0                              | DONE Present                      |
| Connector-side `iatp.sts.oauth` wiring                                           | Connector 0.11.2                       | DONE Present                      |
| Connector-side CS-URL resolution via DID doc `service[]`                         | Connector 0.11.2 DCP runtime extension | NOTE Runtime-check (see section 11.4.R2) |
...
| --- | ----------------------------- | ---------------------------------------------------------------------- | ------ | ------------------------------------------------------------------------------------------------------------ |
| 1   | Create Issuer Participant     | `POST {IS}/api/identity/v1alpha/participants`                          | Issuer | FAIL Missing                                                                                                   |
| 2   | Create Holder Participant     | `POST {IH}/api/identity/v1alpha/participants`                          | Holder | NOTE Has POST but wrong body (see section 11.3)                                                                       |
| 3   | Activate Participant Contexts | `PUT {…}/api/identity/v1alpha/participants/{id}/state?isActive=true`   | Both   | FAIL Missing                                                                                                   |
| 4   | Verify DID Documents          | `GET {…}/.well-known/did.json` (+ per-participant path)                | Both   | NOTE Poll exists but does not verify `service[]` entry                                                         |
| 5   | Create Attestation            | `POST {IS}/api/admin/v1alpha/participants/{id}/attestations`           | Issuer | FAIL Missing                                                                                                   |
| 6   | Create Credential Definition  | `POST {IS}/api/admin/v1alpha/participants/{id}/credentialdefinitions`  | Issuer | FAIL Missing                                                                                                   |
| 7   | Register Holder at Issuer     | `POST {IS}/api/admin/v1alpha/participants/{id}/holders`                | Issuer | FAIL Missing                                                                                                   |
| 8   | Request Credentials (DCP)     | `POST {IH}/api/identity/v1alpha/participants/{id}/credentials/request` | Holder | FAIL **Wrong path used today** — hook POSTs `/api/admin/v1alpha/credentials` which does not exist in IH v0.2.0 |
| 9   | Retrieve Credentials          | `GET {IH}/api/identity/v1alpha/participants/{id}/credentials`          | Holder | FAIL Missing (needed for idempotent re-run + Test Case 1 gating)                                               |
...
DONE Current profile already uses chart defaults. No change.

### 11.6 Updated Phase B (supersedes section 5 Phase B in the original plan)
...
4. **Rewrite the seeding hook** to run all 10 steps with retries, using the
   RBAC + in-pod `kubectl logs` pattern from section 11.3.
...
### 11.7 Amendment to Definition of Done (section 8)

Add:

- [ ] Seeding Job implements all 10 DCP walkthrough steps (section 11.2).
- [ ] Super-user API key sourcing documented and working (section 11.3).
- [ ] DID documents for all 4 participants verified to contain the
      `CredentialService` service entry (section 11.4 R1).
...
If section 11.4 R2 fails (connector 0.11.2 STS request body incompatible with IH
...
compatibility. We will know after Phase B section 5 attempt 1.
...
section 11.8, and it would land in `tractusx-edc` (connector), not in
...
| ------------------------------------------------------- | ------------------------------ | -------------------------------------------------------------------------- |
| Refinement Phase (#1373)                                | Now → **Draft Feature Freeze** | Land section 5 Phase A (config sync), get #1609 PR in draft.                      |
...
```
Week -1 (now)      Finish Phase A wiring (section 5), open draft PR on #1609.
Weeks 0-1          Phase B local run on minikube; fix seeding Job per section 11.2.
...
Week 3             Mark PR ready for review.  Flag section 11.8 tail risk if it materializes.
...
- Per-participant IH topology (Phase D in section 5) → target 26.09.
